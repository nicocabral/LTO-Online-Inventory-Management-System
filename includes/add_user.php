<?php 
	...
	{
		$n = mysql_real_escape_string($_POST['name']);// user name
		$add =mysql_real_escape_string( $_POST['address']);// user email
		$con_no =mysql_real_escape_string( $_POST['contact']);
		$pos 	=mysql_real_escape_string($_POST['position']);
		$off=mysql_real_escape_string($_POST['office']);
		$uname	=mysql_real_escape_string($_POST['username']);
		$pass	=mysql_real_escape_string($_POST['password']);
		$tos	=mysql_real_escape_string($_POST['type_of_user']);
		...
		else
		{
			...
			$upic = rand(1000000,999999).".".$imgExt;
	...
	if($stmt->rowCount() > 0)
	{
		...
		if($pass==$avail or $uname==$availuname){
			echo'<script>window.alert("Username and Password is not available try again!");</script>';
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO user (name,address,contact,position,office,username,password,userpic,user_type) VALUES(:n, :add, :con, :pos, :off, :uname, :pass, :pic, :u_type)');
			$stmt->bindParam(':n',$n);
			$stmt->bindParam(':add', $add);
			$stmt->bindParam(':con', $con_no);
			$stmt->bindParam(':pos', $pos);
			$stmt->bindParam(':off',$off);
			$stmt->bindParam(':uname',$uname);
			$stmt->bindParam(':pass',$pass);
			$stmt->bindParam(':pic',$upic);
